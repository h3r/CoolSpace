...
double Vector3::squaredDistance(){
	return x*x + y*y + z*z; 
}
...
void Vector4::random(float range){
	x = (rand() / (double)RAND_MAX) * 2 * range - range; //value between -range and range
	y = (rand() / (double)RAND_MAX) * 2 * range - range; //value between -range and range
	z = (rand() / (double)RAND_MAX) * 2 * range - range; //value between -range and range
	a = (rand() / (double)RAND_MAX) * 2 * range - range; //value between -range and range
}
void Vector4::random(Vector4 range){
	x = (rand() / (double)RAND_MAX) * 2 * range.x - range.x; //value between -range and range
	y = (rand() / (double)RAND_MAX) * 2 * range.y - range.y; //value between -range and range
	z = (rand() / (double)RAND_MAX) * 2 * range.z - range.z; //value between -range and range
	a = (rand() / (double)RAND_MAX) * 2 * range.a - range.a; //value between -range and range
}
...
{
	return Vector4(a.x + b.x, a.y + b.y, a.z + b.z,  a.a + b.a);
}
...
Vector4 operator * (const Vector4& a, float v)
{
	return Vector4(a.x * v, a.y * v, a.z * v, a.a * v);
}
...
void Matrix44::scale(int x, int y, int z)
{
...
Vector3 Vector3::rotateInAxis(float angle_in_rad, const Vector3& axis)
{
	//mivector x y z
		Matrix44 R;
		R.clear();
