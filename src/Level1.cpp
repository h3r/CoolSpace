...
void Level1::init(){
	...
	_camMini = new Camera();
	_camMini->setPerspective(70, aspect, 0.01, 100000); //set the projection, we want to be perspective
	_camMini->eye = Vector3(0.0, 0.0, 0.0);
	_cam2d = new Camera();
	...
	z = r * cos(altitude);
		...
		CarrierAllied->model.setTranslation(-1000.0, -1500.0, -1000.0);
		CarrierAllied->name = "CarrierAllied";
		CarrierAllied->maxHull = 500;
		CarrierAllied->energy = 500;
		...
		if (s_level == 0){
			CarrierEnemy->mesh = Mesh::LOAD("assets/mesh/NEWCARRIERS/carrier1");
			CarrierEnemy->texture = Texture::LOAD("assets/mesh/NEWCARRIERS/CARRIER01.tga");
		}
		else if (s_level == 1){
			CarrierEnemy->mesh = Mesh::LOAD("assets/mesh/NEWCARRIERS/carrier2");
			CarrierEnemy->texture = Texture::LOAD("assets/mesh/NEWCARRIERS/CARRIER02.tga");
		}
		else{
			CarrierEnemy->mesh = Mesh::LOAD("assets/mesh/NEWCARRIERS/carrier3");
			CarrierEnemy->texture = Texture::LOAD("assets/mesh/NEWCARRIERS/CARRIER03.tga");
		}

		vectorRandom; vectorRandom.random(Vector3(1, 1, 1)); vectorRandom = vectorRandom + Vector3(1.0, 1.0, 1.0); //Rango entre 0 y 2
		azimuth = vectorRandom.x * M_PI;
		altitude = vectorRandom.y * M_PI;
		...
		y = r * sin(azimuth);
		z = r * cos(altitude);
		CarrierEnemy->model.setTranslation(1000.0,1500.0, 1000.0);
		CarrierEnemy->name = "CarrierEnemy";
		CarrierEnemy->maxHull = 100;
		CarrierEnemy->energy = 500;
		CarrierEnemy->maxShield = 100;
	...
	//crear  drones para ambos equipos, el vector en 0.0,0.0,0.0 indica que deben crearse  en sus carriers  determinados
		for (int i = 0; i <10; i++){
			...
			controllers.getControlled(drone->name)->doing = 7;//Atacar al carrier enemigo
		...
		//player->model.setTranslation(2500, 2500, 2500);
		//player->model.setTranslation(10,01,10);
		...
		controllers.getControlled(player->name)->from = player;
...
void  Level1::renderMinimap(Camera* camera){
	std::cout << "camwidth" << camera->width << std::endl;
	glViewport(0, 0, camera->width - 300, camera->height-300);
	//-Pinta lo que sea------------------------------
	/*
	fustrum.ExtractFrustum();
	std::vector<Entity*> * toRender;
...
void Level1::renderGUI(Camera * camera, int state){
	...
	float Shield = ((EntityMovile*)entity)->shield / ((EntityMovile*)entity)->maxShield;//   1.0;//Pon aqui lo que tu quieras
	float Hull = ((EntityMovile*)entity)->hull / ((EntityMovile*)entity)->maxHull;//   1.0;//Pon aqui lo que tu quieras
	entity = getChildren("CarrierAllied");
...
void Level1::update(double elapsed_time){	
	...
	interactions(elapsed_time);
	enemyLogic(elapsed_time);
	...
	//vemos si el jugador a dado alguna orden de produccion
	

}
void Level1::checkCollisions(){
	std::vector<Entity*> asteroidsCheck = children["asteroids"];
	/*
	EntityMovile * player = (EntityMovile*)getChildren("player1");
	for (int i = 0; i < asteroidsCheck.size(); i++){
		EntityMovile* asteroid = (EntityMovile*)asteroidsCheck.at(i);

		Vector3 dir = asteroid->getCenter() - player->getCenter();
		float dist = dir.length() - asteroid->getHalfSize() - player->getHalfSize();

		if (dist < 50){//Colision esfera esfera
			std::cout << "estamos cerca" << dir.length() <<" "<< asteroid->getHalfSize()<< std::endl;
			if (player->testMesh2SphereCollision(asteroid)){
	...
	{
		EntityMovile* to = (EntityMovile*)toCheck.at(i);
		if (!to->alive)continue;
		Vector3 to_pos = to->model*Vector3();
		float to_radius = to->getHalfSize();

		for (unsigned int j = 0; j < bullets->first_inactive_position; j++)
		{
			Bullet* tBullet = bullets->bullets_vector[j];
			...
			//Comprobamos colision con las balas------------------------------------------------------------------------------------------------------
			...
			if (to->name != from->name && (dist<25 ||  to_radius > 100)){
				if (to->testRayCollision(tBullet->last_position, tBullet->position - tBullet->last_position, collisionPoint, true)){
					...
					addChild(emit, "EMITTER");
					if (!to->alive){ s_enemyShipsDestroyed++; 
		...
		//Comprobamos colision con los asteroides-------------------------------------------------------------------------------------------------
		for (int k = 0; k< asteroidsCheck.size();k++)
		{
			EntityMovile * asteroid = (EntityMovile*)asteroidsCheck.at(k);	

			Vector3 dir = asteroid->getCenter() - to->getCenter();
			float dist = dir.length() - asteroid->getHalfSize() - to->getHalfSize();

			if (dist < 0){//Colision esfera esfera
				
				if (to->testMesh2SphereCollision(asteroid)){
					if (!to->alive){ s_alliedShipsDestroyed++; }
				}
			}
		}
		//----------------------------------------------------------------------------------------------------------------------------------------
		...
		for (unsigned int j = 0; j < bullets->first_inactive_position; j++)
		{
			...
			if (to->name != from->name && (dist < 200 || to_radius > 100)){
				if (to->testRayCollision(tBullet->last_position, tBullet->position - tBullet->last_position, collisionPoint, true)){
					...
					//from.score(to.colision(collisionPoint));
					if (!to->alive){
						s_alliedShipsDestroyed++; if (from->name == "player1"){
							s_playerShipsDestroyed++;
						}
					}
				}
			}
	...
	for (unsigned int i = 0; i < bullets->first_inactive_position; i++)
	{
		...
		for (unsigned int j = 0; j < toCheck.size(); j++)
		{
			...
			//Comprobamos colision con los asteroides-------------------------------------------------------------------------------------------------
			for each (Entity* asteroid in asteroidsCheck)
			{
				Vector3 dir = asteroid->model *Vector3() - to_pos;
				float dist = dir.length() - asteroid->getHalfSize() - to_radius;
				if (dist <= 0){//Colision esfera esfera
					if (
						((EntityMovile*)to)->testMesh2SphereCollision(asteroid)){
						//hacer algo con el asteroide, podria depender del tamaño de la otra nave tenga un bump o algo
					}
				}
			}
			//----------------------------------------------------------------------------------------------------------------------------------------
...
void Level1::createAsteroids(Vector3 pos, int qty){
	...
	for (int i = 0; i < qty; i++){
		...
		switch (rand() % 20){
		case 0:{ asteroid->mesh = Mesh::LOAD("assets/mesh/NEWASTEROIDS/hdRoid");	break; }
		case 1:{ asteroid->mesh = Mesh::LOAD("assets/mesh/NEWASTEROIDS/hdRoidMid");	break; }
		case 4: case 5:{ asteroid->mesh = Mesh::LOAD("assets/mesh/NEWASTEROIDS/lqRoid1");	break; }
		case 6: case 7:{ asteroid->mesh = Mesh::LOAD("assets/mesh/NEWASTEROIDS/lqRoid2");	break; }
		case 8: case 9:{ asteroid->mesh = Mesh::LOAD("assets/mesh/NEWASTEROIDS/lqRoid3");	break; }
		default:{ asteroid->mesh = Mesh::LOAD("assets/mesh/NEWASTEROIDS/lqRoid4");	break; }

		}

		asteroid->texture = Texture::LOAD("assets/mesh/hdRoid.tga");
		...
		//asteroid->mesh_low = Mesh::LOAD("assets/mesh/FINALDRONES/LightSpaceShipLow");	asteroid->texture_low = Texture::LOAD("assets/mesh/FINALDRONES/LightSpaceShipLow.tga");
		...
		y = r * sin(azimuth);
		z = r * cos(altitude);
		//asteroid->model.setTranslation(0,0,0);
...
EntityMovile* Level1::createAttacker(std::string team){

	EntityMovile* muhDrone;
	if (!team.compare("alliedTeam")){
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
		controllers.getControlled(muhDrone->name)->doing = 4;
		controllers.getControlled(muhDrone->name)->target = CarrierEnemy;
	}
	else{
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
		controllers.getControlled(muhDrone->name)->doing = 4;
		controllers.getControlled(muhDrone->name)->target = CarrierAllied;
	}

	return muhDrone;
}

EntityMovile* Level1::createDefender(std::string team){

	EntityMovile* muhDrone;
	if (!team.compare("alliedTeam")){
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
		controllers.getControlled(muhDrone->name)->doing = 5;
		controllers.getControlled(muhDrone->name)->target = CarrierAllied;
	}
	else{
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
...
EntityMovile* Level1::createMiner(std::string team){

	EntityMovile* muhDrone;
	if (!team.compare("alliedTeam")){
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
		controllers.getControlled(muhDrone->name)->doing = 7;
		controllers.getControlled(muhDrone->name)->target = CarrierAllied;
	}
	else{
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
...
EntityMovile* Level1::createAtackPlayer(std::string team){

	EntityMovile* muhDrone;
	if (!team.compare("enemyTeam")){
		muhDrone = createDrone(Vector3(0.0, 0.0, 0.0), team);
		controllers.getControlled(muhDrone->name)->doing = 4;
		controllers.getControlled(muhDrone->name)->target = getChildren("player1");;
	}
	return muhDrone;
}
void Level1::interactions(double elapsed_time){
	if (game->dSQUARE){
		constructionMode = 1;
	}
	else if (game->dTRIANGLE){
		constructionMode = 2;
	}
	else if (game->dCIRCLE){
		constructionMode = 3;
	}

	if (constructionMode > 0 && CarrierAllied->energy > 50){
		nextUnit += elapsed_time;
		std::cout << nextUnit << std::endl;
		if (nextUnit > 5.0){
			nextUnit == 0;
			if (constructionMode == 1){
				for (int i = 0; i < 50 / (s_difficulty + 1); i++){
					createMiner("alliedTeam");
				}
				CarrierAllied->energy -= 50;

			}
			else if (constructionMode == 2){
				for (int i = 0; i < 50 / (s_difficulty + 1); i++){
					createDefender("alliedTeam");
				}
				CarrierAllied->energy -= 100;
			}
			else if (constructionMode == 3){
				for (int i = 0; i < 50 / (s_difficulty + 1); i++){
					createAttacker("alliedTeam");
				}
				CarrierAllied->energy -= 200;
			}

			constructionMode = 0;
			nextUnit = 0.0;

			if (CarrierAllied->energy < 0.0){
				CarrierAllied->energy == 0;
			}
		}
	}
...
void Level1::enemyLogic(double Elapsed_time){
	//THE COMPUTER IS A CHEATING BASTARD
	if (s_level == 0){
		enemyCounter += Elapsed_time;
		if (enemyCounter > 30.0){
			enemyCounter = 0.0;
			for (int i = 0; i < 30; i++){
				createAttacker("enemyTeam");
			}
			for (int i = 0; i < 3; i++){
				createAtackPlayer("enemyTeam");
			}
		}
	}
	else if(s_level == 1) {
		...
		enemyCounter += Elapsed_time;
		if (enemyCounter > 30.0 &&  children["enemyTeam"].size()<500){
			enemyCounter = 0.0;
			for (int i = 0; i < 50; i++){
				createAttacker("enemyTeam");
				createDefender("enemyTeam");
...
EntityMovile* Level1::createDrone(Vector3 pos, std::string team, std::string name){
	...
	drone->addChild(emit);
		...
		if (!team.compare("alliedTeam")){
			...
			//drone->mesh = Mesh::LOAD("assets/mesh/FINALDRONES/LightSpaceShip");			drone->texture = Texture::LOAD("assets/mesh/FINALDRONES/LightSpaceShipHigh.tga");
			//drone->mesh_low = Mesh::LOAD("assets/mesh/FINALDRONES/LightSpaceShipLow");	drone->texture_low = Texture::LOAD("assets/mesh/FINALDRONES/LightSpaceShipLow.tga");
		...
		//si es  un enemigo
			...
			//drone->mesh = Mesh::LOAD("assets/mesh/LightSpaceShip");			drone->texture = Texture::LOAD("assets/mesh/LightSpaceShipHigh.tga");
			drone->mesh = Mesh::LOAD("assets/mesh/FINALDRONES/DRONETOM");			drone->texture = Texture::LOAD("assets/mesh/FINALDRONES/DroneTom.tga");
			drone->mesh_low = Mesh::LOAD("assets/mesh/FINALDRONES/DRONETOMLOWPOLY");	drone->texture_low = Texture::LOAD("assets/mesh/FINALDRONES/DroneTom.tga");
