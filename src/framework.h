...
public:
	...
	Vector3 rotateInAxis(float angle_in_rad, const Vector3& axis);
...
public:
	...
	Vector4() { x = y = z = 0.0f; }
	Vector4(float x, float y, float z, float a) { this->x = x; this->y = y; this->z = z; this->a = a; }

	double length();
	double length() const;

	void set(float x, float y, float z, float ) { this->x = x; this->y = y; this->z = z; this->a=a; }

	Vector4& normalize();
	void random(float range);
	void random(Vector4 range);
...
class Matrix44
{
	public:
		...
		void clear();
		void setIdentity();
		void scale(int x_axis,int y_axis,int z_axis);
		void transpose();
