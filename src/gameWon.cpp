...
void gameWon::init(){
	...
	game->sfx[0] = BASS_SampleGetChannel(game->win, false);
	...
	screen->texture = Texture::LOAD("assets/cutscenes/victory.tga");
...
void gameWon::update(double elapsed_time){
	...
	if (shouldExit){
		...
		screenColor.a += elapsed_time*0.50;
...
void gameWon::render(Camera* cam){
	_cam->set();
